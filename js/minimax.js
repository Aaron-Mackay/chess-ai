...
function minimax(game, depth, alpha, beta, isMaximizingPlayer, sum, color) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        if (isMaximizingPlayer) {
            ...
            // sets alpha, the lowest childValue available
            if (childValue > alpha) {
                alpha = childValue;
            }
        }
            ...
            // sets beta, the highest childValue available
            if (childValue < beta) {
                beta = childValue;
            }
        }

        // Alpha-beta pruning -- if alpha is more than beta, this is a deadend, and this branch should be abandoned
        if (alpha >= beta) {
            break;
        }
    }
